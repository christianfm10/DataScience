...
def ir_sunat_web(dni):
    driver = webdriver.Chrome('D:\Proyectos\Python\DataScience\chromedriver.exe')
    # Fijamos nuestra página objetivo
    # driver.get("http://ww1.sunat.gob.pe/cl-ti-itmrconsruc/jcrS00Alias")
    driver.get("https://e-consultaruc.sunat.gob.pe/cl-ti-itmrconsruc/frameCriterioBusqueda.jsp")
    driver.find_element_by_name('search1').send_keys(dni)
    ...
    #         EC.text_to_be_present_in_element_value((By.NAME, "search2"), dni))
    # except:
    #     driver.close()
    #     return []
    try:
        ...
        WebDriverWait(driver, 5).until(
            EC.presence_of_element_located((By.NAME, "imagen")))
        ...
        img_recortada = img.crop(
            (ancho / 1.4, alto / 20, ancho / 1.25, alto / 6.9))
    ...
    #     WebDriverWait(driver, 5).until(
    #         EC.presence_of_element_located((By.CLASS_NAME, "form-table")))
